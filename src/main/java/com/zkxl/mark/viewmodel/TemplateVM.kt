...
class TemplateVM : ViewModel() {
    ...
    val liveData: MutableLiveData<Bean<List<TemplateBean>>> = MutableLiveData()

    fun getTemplateList(body: TemplateRequestBean): MutableLiveData<Bean<List<TemplateBean>>> {

        liveData.postValue(Bean.loading(null))

        RetrofitUtils.getTemplateList(
            body,
            { t ->
                when (t.Code) {
