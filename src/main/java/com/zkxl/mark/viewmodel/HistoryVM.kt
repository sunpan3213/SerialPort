...
class HistoryVM : ViewModel() {
    ...
    val liveData: MutableLiveData<Bean<List<HistoryBean>>> = MutableLiveData()

    fun getHistoryList(map: HashMap<String,String>): MutableLiveData<Bean<List<HistoryBean>>> {

        liveData.postValue(Bean.loading(null))

        RetrofitUtils.getHistoryList(
            map,
            { t ->
                when (t.Code) {
