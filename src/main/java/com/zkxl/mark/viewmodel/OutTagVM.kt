...
import com.zkxl.mark.model.bean.OutRequestBean
 ...
 */
class OutTagVM : ViewModel() {

    val liveData: MutableLiveData<Bean<List<String>>> = MutableLiveData()

    fun out(body: OutRequestBean): MutableLiveData<Bean<List<String>>> {

        liveData.postValue(Bean.loading(null))

        RetrofitUtils.outTag(
            body,
            { t ->
                when (t.Code) {
